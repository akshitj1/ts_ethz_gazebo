...
void GazeboMotorModel::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  if (joint_ == NULL) gzthrow("[gazebo_motor_model] Couldn't find specified joint \"" << joint_name_ << "\".");
  ...
  if (link_ == NULL) gzthrow("[gazebo_motor_model] Couldn't find specified link \"" << link_name_ << "\".");
  ...
  */
  ...
  command_sub_ = node_handle_->Subscribe<mav_msgs::msgs::CommandMotorSpeed>(
	  "~/" + model_->GetName() + command_sub_topic_, &GazeboMotorModel::VelocityCallback, this);
  ...
  motor_velocity_pub_ = node_handle_->Advertise<std_msgs::msgs::Float>(
	  "~/" + model_->GetName() + motor_speed_pub_topic_, 1);
  ...
  ros::SubscribeOptions so =
	  ros::SubscribeOptions::create<std_msgs::Float32>(
		  ...
		  1,
		  boost::bind(&GazeboMotorModel::OnRosMsg, this, _1),
...
*/
void GazeboMotorModel::OnRosMsg(const std_msgs::Float32ConstPtr &_msg) {
  ref_motor_rot_vel_ = std::min(static_cast<double>(_msg->data), static_cast<double>(max_rot_velocity_));
}
...
void GazeboMotorModel::UpdateForcesAndMoments() {
  ...
  ignition::math::Vector3d body_velocity_perpendicular =
	  body_velocity - (body_velocity * joint_axis) * joint_axis;
  ignition::math::Vector3d air_drag =
	  -std::abs(real_motor_velocity) * rotor_drag_coefficient_ * body_velocity_perpendicular;
